...
class HealthFixture(unittest.TestCase):
    ...
    def apply(self, **changes):
        ...
        return apply_probe_result(
            self.conn, self.result(**changes), expected_revision=expected
        )
...
class SchemaTests(HealthFixture):
    def test_schema_is_idempotent_and_has_only_public_states(self):
        ...
        columns = {
            row[1]
            for row in self.conn.execute("PRAGMA table_info(vpn_endpoint_health)")
        }
        self.assertTrue(
            {
                "vpn_id",
        ...
        event_columns = {
            ...
            for row in self.conn.execute(
                "PRAGMA table_info(vpn_endpoint_health_events)"
            )
        }
        self.assertTrue(
            {
    ...
    def test_schema_does_not_mutate_lifecycle_columns(self):
        self.conn.execute(
            "CREATE TABLE vpns(id INTEGER PRIMARY KEY, active INTEGER, onboarding_state TEXT)"
        )
    ...
    def test_partial_legacy_schema_migrates_repeatedly_and_survives_reopen(self):
        self.conn.close()
        with tempfile.TemporaryDirectory() as directory:
            database = Path(directory) / "panel.db"
            conn = sqlite3.connect(database)
            conn.row_factory = sqlite3.Row
            conn.execute("""CREATE TABLE vpn_endpoint_health(
                vpn_id INTEGER PRIMARY KEY,target_revision TEXT NOT NULL,state TEXT NOT NULL,
                consecutive_failures INTEGER NOT NULL,icmp_ok INTEGER,protocol_ok INTEGER,
                protocol_probe TEXT NOT NULL,last_checked_at INTEGER,last_success_at INTEGER,
                last_transition_at INTEGER,updated_at INTEGER NOT NULL)""")
            conn.execute("""CREATE TABLE vpn_endpoint_health_events(
                id INTEGER PRIMARY KEY AUTOINCREMENT,vpn_id INTEGER NOT NULL,old_state TEXT NOT NULL,
                new_state TEXT NOT NULL,icmp_ok INTEGER,protocol_ok INTEGER,
                protocol_probe TEXT NOT NULL,created_at INTEGER NOT NULL)""")
            conn.execute(
            ...
            conn.execute(
                ...
                (1, "accessible", "unreachable", 0, 0, "tcp", 1000),
            )
            conn.commit()
            ...
            try:
                ...
                ensure_endpoint_health_schema(reopened)
                columns = {
                    row[1]
                    for row in reopened.execute(
                        "PRAGMA table_info(vpn_endpoint_health)"
                    )
                }
                self.assertTrue(
                    {
                ...
                health = reopened.execute(
                    "SELECT state,consecutive_failures,last_checked_at FROM vpn_endpoint_health WHERE vpn_id=1"
                ).fetchone()
                self.assertEqual(tuple(health), ("unreachable", 3, 1000))
                event = reopened.execute(
                    "SELECT old_state,new_state,created_at FROM vpn_endpoint_health_events WHERE vpn_id=1"
                ).fetchone()
                self.assertEqual(tuple(event), ("accessible", "unreachable", 1000))
                self.assertEqual(
                    reopened.execute("PRAGMA journal_mode").fetchone()[0], "wal"
                )
            finally:
                reopened.close()
...
class StateMachineTests(HealthFixture):
    ...
    def test_failures_after_unreachable_are_capped_without_new_events(self):
        ...
        self.assertEqual(
            self.health(1240)["consecutive_failures"], MAX_CONSECUTIVE_FAILURES
        )
        self.assertEqual(
            self.conn.execute(
                "SELECT COUNT(*) FROM vpn_endpoint_health_events"
    ...
    def test_revision_change_resets_counter_and_accepts_current_result(self):
        ...
        stored = apply_probe_result(
            self.conn, result, expected_revision=new_revision, expected_generation=2
        )
    ...
    def test_result_revision_mismatch_is_rejected_without_mutation(self):
        ...
        with self.assertRaises(StaleRevisionError):
            apply_probe_result(
                self.conn,
                self.result(target_revision=other),
    ...
    def test_cycle_duplicate_is_idempotent_and_older_cycle_is_rejected(self):
        first = self.apply(
            checked_at=1000, icmp_ok=True, cycle_id=4, lease_id="lease-a"
        )
        duplicate = self.apply(
            checked_at=1100, icmp_ok=False, cycle_id=4, lease_id="lease-a"
        )
...
class ValidationAndHistoryTests(HealthFixture):
    ...
    def test_only_transitions_are_stored_and_history_uses_transition_evidence(self):
        ...
        self.assertEqual(
            {item["state"] for item in intervals}, {"accessible", "unreachable"}
        )
        self.assertTrue(all(item["start_at"] < item["end_at"] for item in intervals))
        self.assertTrue(
            any(
                item["state"] == "unreachable" and item["protocol_ok"] is False
    ...
    def test_tuple_and_row_legacy_migrations_preserve_timestamp_and_diagnostics(self):
        for row_factory in (None, sqlite3.Row):
            ...
            conn.execute(
                "INSERT INTO vpn_endpoint_health VALUES(1,?,?,?,?,?,?,?,?)",
                (
                    "a" * 64,
                    "tcp_connect",
                    "down",
            ...
            ensure_endpoint_health_schema(conn)
            row = conn.execute(
                "SELECT state,protocol_probe,protocol_ok,last_checked_at,protocol_code FROM vpn_endpoint_health WHERE vpn_id=1"
            ).fetchone()
            self.assertEqual(
                tuple(row), ("unreachable", "tcp", 0, 1000, "tcp_unreachable")
            )
            conn.close()
...
class AppIntegrationTests(unittest.TestCase):
    def test_app_initializes_endpoint_health_schema(self):
        ...
        imported = any(
            ...
            and any(
                alias.name == "ensure_endpoint_health_schema" for alias in node.names
            )
